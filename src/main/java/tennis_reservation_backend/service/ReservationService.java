...
public class ReservationService {
    ...
    // ★ 予約作成処理：nullチェックガードの強化 & コートステータス更新
    @Transactional
    public Reservation save(Reservation reservation) {
        // 0. 必須IDパラメータの null チェックガード（Hibernate事前バリデーションエラー防止）
        if (reservation.getCourtId() == null) {
            throw new IllegalArgumentException("コートID（courtId）が指定されていません。");
        }
        if (reservation.getUserId() == null) {
            throw new IllegalArgumentException("ユーザーID（userId）が指定されていません。");
        }

        // 1. 対象コートのステータスチェック（available 以外は予約不可）
        Court court = courtRepository.findById(reservation.getCourtId())
                .orElseThrow(() -> new IllegalArgumentException("指定されたコートが存在しません。ID: " + reservation.getCourtId()));
